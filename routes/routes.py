...
def add():
    ...
    try:
        assert isinstance(data["param1"], int) or isinstance(data["param1"], float)
        assert isinstance(data["param2"], int) or isinstance(data["param2"], float)
        
    except Exception:
        return jsonify({
            "result": None,
            "meta": {"error": "Operands param1 and param2 should be either an int or a float"}
        }), HTTPStatus.BAD_REQUEST
